...
class TestDrift(unittest.TestCase):
    ...
    def test_types(self):
        ...
        for invalid in [[], None, 0, tuple([0]), (0, 1)]:
            with self.assertRaises(ValidationError):
                Drift(gamma=1.0, direction_normalized=invalid)

        for invalid in [[], None, (1, 2, 3), tuple([0]), (0, 1)]:
            with self.assertRaises(ValidationError):
                Drift(gamma=invalid, direction_normalized=(1, 0, 0))

    def test_invalid_gamma(self):
        """invalid values for gamma are rejected"""
        for invalid in [-1, -123.3, 0, 0.9999999999, math.inf, math.nan]:
            with self.assertRaises(ValueError):
                Drift(direction_normalized=(1, 0, 0), gamma=invalid)
    ...
    def test_fill_from_invalid(self):
        ...
        for invalid in invalid_inputs:
            for permutation in itertools.permutations(invalid):
                ...
                with self.assertRaises(ValueError):
                    Drift.from_velocity(vector3)
                with self.assertRaises(ValueError):
                    Drift.from_gamma_velocity(vector3)

    def test_fill_from_velocity(self):
        """computation based on velocity vector"""
        d = Drift.from_velocity((1742, 1925, 1984))
        ...
        d = Drift.from_velocity((41782731.0, 61723581.0, 212931235.0))
        ...
        for components in faster_than_light_list:
            for ftl in itertools.permutations(components):
                ...
                with self.assertRaisesRegex(ValueError, ".*[Ll]ight.*"):
                    Drift.from_velocity(ftl)

    def test_fill_from_gamma_velocity(self):
        """computation based on velocity vector multiplied with gamma"""
        d = Drift.from_gamma_velocity((29379221.65264335, 141390308.68517736, 265336.4756518417))
        ...
        d = Drift.from_gamma_velocity((359876252.1771755, 42747107.177341804, 43958755.5088825))
        ...
        d = Drift.from_gamma_velocity((1, 0, 0))
