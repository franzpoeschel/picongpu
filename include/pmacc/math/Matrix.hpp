/* Copyright 2023-2024 Brian Marre, Rene Widera
 *
...
//! @file implementation of matrix and matrix multiplication
...
{
    /** 2 d matrix matrix definition
     *
     * @tparam T_Type data type used for storage of elements
     * @tparam T_Extent pmacc::math::CT::Vector, two dimensional vector describing extent of matrix in each dimensions
     *
     * @attention uses mathematical indexing
     */
    template<typename T_Type, typename T_Extent>
    struct Matrix
    {
        PMACC_CASSERT_MSG(matrix_must_have_2_dimensional_extent, T_Extent::dim == 2);
    ...
    public:
        ...
        /** access via (m, n), using mathematical notation
         *
            ...
            Matrix<T_Type, T_ExtentRhs> const& rhs) const
        {
            ...
            for(uint32_t j = 0u; j < jExtent; ++j)
            {
                ...
                for(uint32_t i = 0u; i < iExtent; ++i)
                {
                    ...
                    constexpr auto kExtent = n;
                    PMACC_UNROLL(kExtent)
                    for(uint32_t k = 0u; k < kExtent; ++k)
                    {
