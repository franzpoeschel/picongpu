...
* Backend-specific runtime parameters may be set via JSON or TOML in the openPMD API.
  PIConGPU exposes this via the command line option ``--openPMD.json``.
  The name of the command line flag is historical, the inline value or file referenced as value may be either JSON or TOML.
  Please refer to the `openPMD API's documentation <https://openpmd-api.readthedocs.io/en/dev/details/backendconfig.html>`_ for further information.

The JSON/TOML parameter may be passed directly as a string, or by filename.
...
The openPMD-api has a special configuration markup that may optionally be used in order to pass **dataset-specific configurations**.
For each backend ``<backend>``, the backend-specific dataset configuration found under ``config["<backend>"]["dataset"]`` may have one of two forms:

1. A JSON object containing the backend-specific configuration for all datasets.
2. A JSON list of patterns: ``[<pattern_1>, <pattern_2>, …]``.

In the second case, each pattern ``<pattern_i>`` is a JSON object with key ``cfg`` and optional key ``select``: ``{"select": <pattern>, "cfg": <cfg>}``.

In here, ``<pattern>`` is a regex or a list of regexes, as used by POSIX ``grep -E``.
``<cfg>`` is the configuration to be applied to those datasets that match the specified naming pattern.
...
The position of the default case within the pattern list does not matter, it will always be treated as the fallback, i.e. the last entry.
