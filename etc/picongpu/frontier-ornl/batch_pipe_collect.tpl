...
.TBG_nodes_pic="$((( TBG_tasks + TBG_devicesPerNode - 1 ) / TBG_devicesPerNode))"
.TBG_fan_in_factor=64
.TBG_nodes_pipe="$((( TBG_nodes_pic + TBG_fan_in_factor - 1 ) / TBG_fan_in_factor))"
.TBG_nodes_both="$(( TBG_nodes_pic + TBG_nodes_pipe ))"
...
echo "nodes pic: !TBG_nodes_pic, nodes pipe: !TBG_nodes_pipe, nodes both: !TBG_nodes_both, nodes oversubscription: !TBG_nodes_adjusted"


# adjust number of nodes for fault tolerance adjustments
.TBG_nodes_adjusted=$((!TBG_nodes_both * (1000 + !TBG_node_oversubscription_pt) / 1000))
.TBG_tasks_adjusted=$((!TBG_nodes_adjusted * !TBG_numHostedDevicesPerNode))

## end calculations ##

echo 'Start job with !TBG_nodes_adjusted nodes. Required are !TBG_nodes_both nodes.'
...
set -x
srun --ntasks-per-node=1 $exclude_nodes --nodes=!TBG_nodes_both hostname > all_hostnames.txt
set +x

awk '(NR - 1) % (!TBG_fan_in_factor + 1) == 0 {print}' < all_hostnames.txt > pipe_hostnames.txt
awk '(NR - 1) % (!TBG_fan_in_factor + 1) != 0 {print}' < all_hostnames.txt > pic_hostnames.txt
...
export OPENPMD_CHUNK_DISTRIBUTION=fan_in
export OPENPMD_FAN_IN=!TBG_fan_in_factor
...
if [ $node_check_err -eq 0 ] || [ $run_cuda_memtest -eq 0 ] ; then
    ...
    srun -l                                       \
      --nodes !TBG_nodes_pipe                     \
      --nodelist=./pipe_hostnames.txt             \
      --ntasks-per-node=!TBG_fan_in_factor        \
      --ntasks="$((!TBG_fan_in_factor * !TBG_nodes_pipe))" \
      --gpus-per-node=0                           \
